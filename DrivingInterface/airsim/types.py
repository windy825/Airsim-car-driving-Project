...
class getAlgoUserAPI(MsgpackMixin):
    wayPoints = []
    ac_RaceComplete_YN = False
    ac_block_points = []
    ac_player_current_lap = 1
    settings_path = ""
    ac_respawn_yaws = []
    ac_road_width_half = 0.0
    ac_player_lap_progress = 0.0
    ac_rematch_YN = False

class getAlgoAdminAPI(MsgpackMixin):
    ac_RaceStart_YN = False
    ac_PlayerRaceTimes = []
    ac_player_first_lap_time = 0.0

class setResetLocation(MsgpackMixin):
    setResetLocation = False

class input_player_lap_progress(MsgpackMixin):
    player_lap_progress = True

class ac_rematch_check(MsgpackMixin):
    ac_rematch = False
