...
class VehicleClient:
    ...
    def simLap_Time(self):
        return self.client.call("simLap_Time")
...
class CarClient(VehicleClient, object):
    ...
    def getAlgoUserAPI(self, vehicle_name = ''):
        algo_user_apis = self.client.call('getAlgoUserAPI', vehicle_name)
        return CarState.from_msgpack(algo_user_apis)

    def getAlgoAdminAPI(self, vehicle_name = ''):
        algo_admin_apis = self.client.call('getAlgoAdminAPI', vehicle_name)
        return CarState.from_msgpack(algo_admin_apis)

    def setResetLocation(self, x_pos, y_pos, yaw, vehicle_name = ''):
        algo_setResetLocation = self.client.call('setResetLocation', x_pos, y_pos, yaw, vehicle_name)
        return CarState.from_msgpack(algo_setResetLocation)

    def input_player_lap_progress(self, curr_lap_progress, vehicle_name = ''):
        algo_input_player_lap_progress = self.client.call('input_player_lap_progress', curr_lap_progress, vehicle_name)
        return CarState.from_msgpack(algo_input_player_lap_progress)

    def ac_rematch_check(self, vehicle_name=''):
        algo_ac_rematch_check = self.client.call('ac_rematch_check', vehicle_name)
        return CarState.from_msgpack(algo_ac_rematch_check)