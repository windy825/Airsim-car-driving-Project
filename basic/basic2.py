...
class DrivingClient(DrivingController):
    ...
    def control_driving(self, car_controls, sensing_info):
        ...
        y = ways[tg][0] if ways[tg][0] else 0.1
        x = ways[tg][1] if ways[tg][1] else 0.1
        theta = math.atan(x / y) * 180 / math.pi - sensing_info.moving_angle

        if abs(angles[tg+2]) < 49:
            if abs(angles[-1]) > 90 and spd > 165:
                if angles[-1] * plag >= 0:
                    theta = math.atan((x + 9) / y) * 180 / math.pi - sensing_info.moving_angle
                else:
                    theta = math.atan((x - 9) / y) * 180 / math.pi - sensing_info.moving_angle
            ...
            r = max(abs(x), abs(y))
            alpha = math.asin(math.sqrt(x ** 2 + y ** 2) / (2 * r)) * 2
